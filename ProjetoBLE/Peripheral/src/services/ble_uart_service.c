...
static ssize_t write_chars(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buf, uint16_t size, uint16_t offset, uint8_t flags)
{
	...
	if (size > sizeof(chrc_data))
	{
		printk("Invalid size\n");
		return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
	} 
	else if (offset + size > sizeof(chrc_data))
	{
		printk("Invalid offset and size\n");
	...
	(void)memcpy(chrc_data + offset, buf, size);
	prepare_count = 0;

    if(rx_callback)
        rx_callback((const uint8_t *)buf, size);
...
static void ccc_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	if (notif_enabled) SFLAG(flag_long_subscribe);
...
int service_transmit(const uint8_t *buffer, size_t size)
{
    printk("\nTransmitting...\n");
    ...
    for(i = 0; i < size; i++)
	{
        if(buffer[i] >= 'a' && buffer[i] <= 'z') text[i] = buffer[i] - 32;
        else text[i] = buffer[i];
    }

    text[size] = '\0';

    if(conn) return (bt_gatt_notify(conn, &attrs[2], text, size));
    else return -1;
}